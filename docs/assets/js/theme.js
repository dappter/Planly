document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyMainContentStyles() {
    document
      .querySelectorAll("p:not(.button), h1:not(.button)")
      .forEach((element) => {
  ...
  function handleButtonStyles() {
    document.querySelectorAll(".button").forEach((button) => {
      ...
      if (button.id === "saibaMais" || button.id === "gerarExemplo") {
        ...
        buttonHoverListeners.set(button, {
          over: mouseoverListener,
          out: mouseoutListener,
        });
      } else {
        const mouseoverListener = () =>
  ...
  function setDarkTheme() {
    ...
    // Preservar elementos importantes ao remover estilos
    const progressElements = document.querySelectorAll(
      '[id*="progress"], .habit-progress-fill, #xpProgressBar',
    );
    const savedStyles = new Map();

    progressElements.forEach((el) => {
      if (el.style.width) {
        savedStyles.set(el, el.style.width);
      }
    });

    document.querySelectorAll("[style]").forEach((element) => {
      // Preservar loading, barra de progresso e barras de hábitos
      if (element.id === "loading" && element.style.display === "none") {
        element.style.display = "none";
      } else if (
        element.id === "xpProgressBar" ||
        element.className?.includes("progress") ||
        element.id?.includes("progress")
      ) {
        // Não remover estilos de barras de progresso
        return;
      } else {
    ...
    // Restaurar larguras das barras de progresso
    savedStyles.forEach((width, element) => {
